...
function analyse(prov, svc, seen = new Set()) {
  ...
  const re = /^ {2}(?:(?:public|private|protected)\s+)?(?:async\s+)?([a-z]\w*)\s*[(<]/gm;
  for (const m of src.matchAll(re)) {
    const name = m[1];
    ...
    const helperCall = [...helpers].some((h) =>
      new RegExp(`^(?:return\\s+|throw\\s+)?${h}\\([^;]*\\);?$`, 's').test(inner),
    );
...
console.log('-'.repeat(74));
console.log(
  pad('TOTAL', 13) + pad(total, 9) + PROVIDERS.map((p) => pad(`${per[p].impl} + ${per[p].uns}X`, 13)).join(''),
);
const impl = PROVIDERS.reduce((a, p) => a + per[p].impl, 0),
  uns = PROVIDERS.reduce((a, p) => a + per[p].uns, 0),
  miss = PROVIDERS.reduce((a, p) => a + per[p].miss, 0);
console.log(
