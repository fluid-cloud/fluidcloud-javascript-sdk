...
function makeProviderModule(provider: string): Record<string, unknown> {
  const names = [
    'S3Storage', 'SecretsManager', 'ParameterStore', 'SnsMessaging', 'SqsQueue', 'SesEmail',
    'CloudWatchMonitoring', 'CloudTrailAudit', 'KinesisStreaming', 'MskStreaming', 'CloudFrontCdn',
    'CognitoIdentity', 'ElastiCacheRedis', 'OpenSearchAws',
    'BlobStorage', 'KeyVaultSecrets', 'AppConfigurationParameters', 'ServiceBusMessaging',
    'ServiceBusQueue', 'AcsEmail', 'AzureMonitoring', 'ActivityLogAudit', 'EventHubsStreaming',
    'FrontDoorCdn', 'EntraIdentity', 'AzureRedisCache', 'AiSearch',
    'GcsStorage', 'SecretManagerSecrets', 'ParameterManagerParameters', 'PubSubMessaging',
    'PubSubQueue', 'GcpEmail', 'CloudMonitoring', 'CloudAudit', 'PubSubStreaming', 'CloudCdn',
    'CloudIdentity', 'MemorystoreRedis', 'GcpSearch',
    'ObjectStorage', 'VaultSecrets', 'VaultParameters', 'OnsMessaging', 'OciQueue', 'OciEmail',
    'OciMonitoring', 'OciAudit', 'OciStreaming', 'OciCdn', 'IamDomainsIdentity', 'OciRedisCache',
    'OpenSearchOci',
  ];
...
describe('createClient', () => {
  ...
  it('creates every always-on service for AWS with no options', async () => {
    ...
    expect([
      client.hasStorage, client.hasSecrets, client.hasParameters, client.hasMessaging,
      client.hasQueue, client.hasEmail, client.hasMonitoring, client.hasAudit,
      client.hasStreaming, client.hasCdn, client.hasIdentity,
  ...
  it.each([
    ...
    ['storage with storageAccount', { storageAccount: 'acct' }, (c: Awaited<ReturnType<typeof createClient>>) => c.hasStorage, true],
    ['secrets without keyVaultName', {}, (c: Awaited<ReturnType<typeof createClient>>) => c.hasSecrets, false],
    ['secrets with keyVaultName', { keyVaultName: 'kv' }, (c: Awaited<ReturnType<typeof createClient>>) => c.hasSecrets, true],
    ['queue without serviceBusNamespace', {}, (c: Awaited<ReturnType<typeof createClient>>) => c.hasQueue, false],
    ['queue with serviceBusNamespace', { serviceBusNamespace: 'sb' }, (c: Awaited<ReturnType<typeof createClient>>) => c.hasQueue, true],
    ['cdn without cdnProfileName', {}, (c: Awaited<ReturnType<typeof createClient>>) => c.hasCdn, false],
    ['cdn with cdnProfileName', { cdnProfileName: 'fd' }, (c: Awaited<ReturnType<typeof createClient>>) => c.hasCdn, true],
    ['streaming without eventHubsNamespace', {}, (c: Awaited<ReturnType<typeof createClient>>) => c.hasStreaming, false],
    ['streaming with eventHubsNamespace', { eventHubsNamespace: 'eh' }, (c: Awaited<ReturnType<typeof createClient>>) => c.hasStreaming, true],
